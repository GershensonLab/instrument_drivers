...
#        dll_path = 'C:\\WINDOWS\\system32\\ATS_Average_DLL.dll'  #additional dll for specific needs (ie GetAverregedBuffer)
        ...
        super().__init__(name, dll_path=dll_path, alt_dll_path=alt_dll_path, **kwargs)
        # add parameters
...
#                    print('\nATS_StartCapture \t', time.time() - start)                      
#                    start = time.time()
...
#                        print('ATS_GetAverageBuffer \t', time.time() - start) 
#                        start = time.time()
...
#                        print('ATS_StopCapture \t', time.time() - start) 
#                        start = time.time()    
    ...
    def get_prepared(self, N_pts, N_avg, N_buf = 1):
        ...
        self._call_dll('ATS_ConfigCapture',  #from ATS_Average_NPT_custom_DLL
                          self._handle,
                          channelMask,
        ...
        try:
            ...
            self._call_dll('ATS_StopCapture',  #from ATS_Average_NPT_custom_DLL
                                  self._handle)
            self.Prepared = False
#            print('\n Card resources were released\n')
    ...
    def get_averaged(self):   
        ...
        if self.Prepared:
            ...
            try:
                while (buffers_completed < number_of_buffers):
                    ...
                    self._call_dll('ATS_GetAverageBuffer',     #from ATS_Average_NPT_custom_DLL
                                  self._handle,
                                  cast(buf.addr, c_void_p),
                                  bufferSize_samples,
                                  timeout_ms)
        ...
        return output / self.number_of_buffers
