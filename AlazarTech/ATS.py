...
#    alt_dll_path = 'C:\\WINDOWS\\System32\\ATSApi' ##alt (for some specific needs) dll to use
    ...
    def __init__(self, name: str, system_id: int=1, board_id: int=1,
                 dll_path: str=None, AVG_dll_path: str=None, TD_dll_path: str=None, **kwargs) -> None:
        super().__init__(name, **kwargs)

        if os.name == 'nt':
            self._ATS_dll = ctypes.cdll.LoadLibrary(dll_path or self.dll_path)

            if AVG_dll_path is not None:
                self._AVG_dll = ctypes.cdll.LoadLibrary(AVG_dll_path or self.AVG_dll_path) #add dll for avg functions#

            if TD_dll_path is not None:
                self._TD_dll = ctypes.cdll.LoadLibrary(TD_dll_path or self.TD_dll_path) #add dll for td functions#
    ...
    def acquire(self, mode=None, samples_per_record=None,
                ...
                allocated_buffers=None, buffer_timeout=None, dmaBufferCount = None,
                number_of_averaging = None,
        ...
        self._set_if_present('dmaBufferCount', dmaBufferCount)

        # endregion
        mode = self.mode.get()
        if mode not in ('TS', 'NPT', 'AVG'):
...
#            recordsPerBuffer = self.records_per_buffer.raw_value
            ...
            number_of_averaging = self.number_of_averaging.raw_value
            timeout_ms = self.buffer_timeout.raw_value
    #        channelMask = 3 
    ##        samplesPerRecord = 8192
    #        samples_per_record = 8192
    #        records_per_buffer = 4000
    #        numberofBuffers = 1
    #        dmaBufferCount = 2
            ...
            print(' bytes_per_buffer', bytes_per_buffer )
            print(' bytes_per_sample', bytes_per_sample )
    
            print(' bytes_per_buffer //  bytes_per_sample', bytes_per_buffer //  bytes_per_sample )
...
#        return output
    ...
    def _call_dll(self, pre_func_name: str, *args) -> None:
        ...
        Dll is specified in the prefix, TD for timedomain, AVG for averaging
        ...
        if pre_func_name[:3] == 'TD_':
            try:
                dll = self._TD_dll
                func_name = pre_func_name[3:]
            except AttributeError:
                print ('To use TD_* functions you have to provide TD_dll at card initiation')

        elif pre_func_name[:4] == 'AVG_':
            try:
                dll = self._AVG_dll
                func_name = pre_func_name[4:]
            except AttributeError:
                print ('To use AVG_* functions you have to provide AVG_dll at card initiation')
        else:
                dll = self._ATS_dll
...
#            func = getattr(dll, func_name)
#        except AttributeError:
#            func = getattr(self._ATS_alt_dll, func_name)    # try alternate dll if havnt found called function im the main dll
#        finally:
#            print('\nFuntction {}, args {}'.format( func_name,  repr(args_out) ))
...
#        print(' bytes_per_sample', bytes_per_sample )
#
#        print(' size_bytes //  bytes_per_sample',size_bytes //  bytes_per_sample )
#         
...
class AcquisitionController(Instrument):
    ...
    """
    def __init__(self, name, alazar_name,alazar_model,  **kwargs):
        """
        Args:
            alazar_name: The name of the alazar instrument on the server
        """
        super().__init__(name, **kwargs)
        self.model = alazar_model
        self._alazar = self.find_instrument(alazar_name,
                                            instrument_class=self.model)
#        
